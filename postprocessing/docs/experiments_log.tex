...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_expA_ssh_snapshots.png}
...
\begin{itemize}
    \item \textbf{Significato Fisico}: La varianza è una misura dell'energia dell'onda in quel punto ($\text{Var} \propto \langle \eta^2 \rangle$). Le zone rosse (alta varianza) corrispondono agli antinodi, dove l'oscillazione verticale è massima (lungo le coste).
    \item \textbf{Punto Anfidromico}: La zona blu scuro al centro rappresenta un minimo globale di varianza (quasi zero). Questo è il \textit{Punto Anfidromico}, un nodo immobile attorno al quale l'onda di marea/Kelvin ruota. In questo punto, l'elevazione della superficie rimane costante mentre l'onda gli gira attorno.
\end{itemize}
\begin{figure}[H]
    \centering
