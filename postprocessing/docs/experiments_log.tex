...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Risoluzione Orizzontale: $\Delta x \approx 10$ km (\texttt{nn\_GYRE = 1}).
        \item Risoluzione Verticale: 31 livelli ($z$-coordinates).
    \end{itemize}
    \item \textbf{Perturbazione Iniziale}:
    \begin{itemize}
        \item Tipo: Anomalia Gaussiana di SSH.
        \item Ampiezza: $A_0 = 1.0$ m.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{fig_expA_ssh_variance.png}
...
Il percorso di estrazione per il diagramma di Hovmöller scorre lungo la costa orientale, come mostrato in Figura \ref{fig:pathA}.
Il diagramma lungo la costa Est (Figura \ref{fig:hovmollerA}) mostra una propagazione lineare senza distorsioni.
\begin{figure}[H]
    \centering
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[height=5.5cm]{fig_expA_path.png}
        \caption{Percorso di estrazione (linea rossa) per il diagramma di Hovmöller (Exp A).}
        \label{fig:pathA}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[height=5.5cm]{fig_expA_hovmoller_east.png}
...
Abbiamo confrontato le serie temporali dell'SSH registrate all'Estremità Nord del bacino, normalizzate per l'ampiezza iniziale $A_0$.
...
\begin{figure}[H]
    ...
    \caption{Confronto delle serie temporali normalizzate all'Estremità Nord. La sovrapposizione indica una risposta lineare.}
    \label{fig:linearity}
\end{figure}

Per quantificare eventuali effetti non lineari e identificare il punto anfidromico, mostriamo le analisi spaziali in Figura \ref{fig:expB_maps}.
...
La mappa a destra mostra la varianza totale del segnale, evidenziando (più o meno) il minimo centrale (punto anfidromico) attorno al quale l'onda ruota.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \label{fig:variance}
    \end{minipage}
    \caption{Analisi Spaziale Exp B.}
    \label{fig:expB_maps}
\end{figure}
...
\begin{itemize}
    ...
    \textit{Nota: La profondità minima è stata portata a 100m (inizialmente 30m) per evitare problemi di risoluzione verticale. Con 31 livelli ($jpk=31$), ogni livello è alto circa 33m; una profondità di 30m avrebbe comportato un singolo livello, causando problemi.}
    
    La visualizzazione del dominio è in Figura \ref{fig:bathyC}.
    \item \textbf{Perturbazione}: $A_0 = 0.1$ m (Regime Lineare) centrata a Sud-Est.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{fig_ExpC_bathymetry_3D.png}
...
$$ A \propto H^{-1/4} $$
Confrontando la serie temporale dell'SSH all'Estremità Nord tra l'esperimento C (30m) e la Baseline (100m) in Figura \ref{fig:shoaling}, osserviamo chiaramente:
\begin{enumerate}
...
Aumentando la risoluzione, ci aspettiamo che l'onda mantenga meglio la sua forma e ampiezza nel tempo, riducendo la viscosità numerica implicita.
...
\begin{figure}[H]
    ...
    \caption{Diagramma di Hovmöller per Exp D (5km). Notare la nitidezza del fronte d'onda.}
    \label{fig:hov_D}
\end{figure}
