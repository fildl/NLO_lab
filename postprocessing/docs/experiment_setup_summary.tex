...
\textbf{D} & 2 ($\Delta x \approx 5$km) & 20 & Piatta 100m & Media Risoluzione (CFL Sicuro). \\ \hline
\textbf{E} & 1 ($\Delta x \approx 10$km) & 60 & Piatta 100m & $f=0$. Validazione Fisica (No Coriolis). \\ \hline
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Exp E}: Mofidicato \texttt{usrdef\_hgr.F90} per forzare \texttt{pff\_f = pff\_t = 0} (No Coriolis). Ripristinato \texttt{namelist\_cfg} alla Baseline.
\end{itemize}
