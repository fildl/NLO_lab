...
\institute{Università di Bologna \\ Numerical Laboratory of Atmosphere and Ocean}
\date{\today}
...
\begin{frame}{Exp A: Diagramma di Hovmöller}
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            \begin{figure}
                \centering
                \only<1>{\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{../postprocessing/fig_expA_hovmoller_east.png}}%
                \only<2>{\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{../postprocessing/fig_expA_hovmoller_east_theory.png}}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Exp A: Punto Anfidromico}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            La mappa della varianza temporale dell'SSH identifica le zone di massima escursione (antinodi) e di minima (nodi).
        \end{column}
        \begin{column}{0.6\textwidth}
            \begin{figure}
                \centering
                \only<1>{\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{../postprocessing/fig_expA_ssh_variance.png}}%
                \only<2>{\includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{../postprocessing/fig_expA_ssh_variance_contour.png}}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Exp B: Sensibilità all'Ampiezza}
    \textbf{Domanda:} Il modello risponde linearmente aumentando l'ampiezza iniziale?
    \begin{itemize}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.65\textwidth]{../postprocessing/fig_expB_compare_linearity.png}
    \end{figure}
\end{frame}
...
\begin{frame}{Exp C: Effetto della Batimetria}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.9\textwidth]{../postprocessing/fig_ExpC_bathymetry_3D.png}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Exp C: Confronto Velocità di Fase}
    L'onda su fondale profondo (Exp C, linea Rossa) arriva a destinazione molto prima rispetto alla Baseline (linea Nera).

    \begin{figure}
        \centering
        \includegraphics[width=0.75\textwidth]{../postprocessing/fig_ExpC_shoaling_comparison.png}
    \end{figure}
\end{frame}
...
\begin{frame}{Exp C: Dinamica e Raggio di Rossby}
    ...
    \end{itemize}
    In acqua profonda il raggio di Rossby supera la larghezza del bacino: l'onda perde il carattere costiero "stretto".
    
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{../postprocessing/fig_ExpC_snapshots.png}
    \end{figure}
\end{frame}
